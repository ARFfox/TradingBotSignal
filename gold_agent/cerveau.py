...
# Le consensus vit dans consensus.py (regle 17)
from .consensus import POIDS_TF, _consensus  # noqa: F401


def _etat_boucles(problemes: list, agents_sante: list) -> None:
    """Ajoute l'etat des boucles permanentes a la sante : une boucle morte
    est un PROBLEME signale, jamais masque (contrat de la spec §6)."""
    try:
        from . import boucles
        rows = boucles.etat()
    except Exception:
    ...
    details = []
    for b in rows:
        age = b["battement_age_s"]
        if not b["vivante"]:
            problemes.append(
                f"boucle « {b['nom']} » sans battement depuis "
                f"{age if age is not None else '∞'} s "
                f"(intervalle {b['intervalle']:.0f} s)"
                + (f" — {b['derniere_erreur']}" if b["derniere_erreur"] else ""))
        details.append(f"{b['code']} : battement il y a {age or '—'} s · "
                       f"{b['tours']} tours · {b['derniere_duree_ms']} ms")
    agents_sante.append({
        "nom": "Boucles permanentes", "emoji": "🫀",
        "role": "collecte & signaux — toujours en ligne, meme sans navigateur",
        "source": "battements de coeur (3 x intervalle = morte)",
        "detail": " · ".join(details),
        "ok": all(b["vivante"] for b in rows), "cible": None})
...
def _sante(resultats: list, usage, quote) -> dict:
    ...
    _etat_boucles(problemes, agents)
