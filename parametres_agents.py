...
from statistiques import marge_bruit
...
def evaluer_parametre(
    ...
    # La marge exigée dépend de COMBIEN de valeurs ont été essayées.
    #
    # Compter les découpes favorables ne suffit pas : 4 découpes sur 4
    # arrivent par pur hasard dans 6 % des cas, et neuf valeurs essayées
    # suffisent à tomber dessus une fois sur cinq. C'est exactement le
    # sur-apprentissage que ce module existe pour empêcher — et il passait
    # par sa propre porte.
    n_candidats = max(1, len(param.candidats()))
    seuil = max(MARGE_BRUIT, marge_bruit(gains, n_candidats))
    ...
    if prop == param.valeur:
        ...
        ok, motif = False, (
            f"gain {gain:+.3f}R sous la marge exigée ({seuil:.3f}R, "
            f"corrigée pour {n_candidats} valeurs essayées)")
